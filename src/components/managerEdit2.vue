                    ...
                    <template v-for="(item,index) in tableHead">
                      <el-table-column :prop="item.cid" :label="item.cname" align="center" :formatter="showJudge">
                        <template slot-scope="scope">
                          <el-input
                            readonly
                            class="paperview-input-text" v-model="scope.row[scope.column.property]">
                          </el-input>
                        </template>
                      </el-table-column>
                    </template>
                  ...
                  <el-table-column
                    ...
                    <template slot-scope="scope">
                      <el-input class="paperview-input-text"
                                ...
                                @blur="editSum2(scope.row.sumscore)"
                      ></el-input>
                    </template>
                  </el-table-column>
                </el-table>
    ...
    methods: {
      ...
      editSum2(sumscore){
        if (sumscore<0||sumscore>100){
          ...
          axios.get("http://localhost:8081/updateApp02?sumscore="+sumscore+"&&sid="+this.student.sid+"&&type="+2).then(res => {
            if (res.data==1){
      ...
      scoreEdit2(cname, score) {
        if (score < 0 || score > 100) {
          ...
          axios.get("http://localhost:8081/updateEmpScore?cname="
            + cname + "&&score=" + score + "&&eid=" + this.eid + "&&type=" + 2
